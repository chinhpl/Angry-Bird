                ...
                const long &volume[], const int &spread[])
{
    ...
    ArraySetAsSeries(typical_price, false);
    //--- initial zero
    ...
    {
        typical_price[i] = (low[i] + high[i] + close[i]) / 3;
        //--- middle line
        ExtMovingBuffer[i] = SimpleMA(i, InpBandsPeriod, typical_price);
        //--- calculate and write down StdDev
        ExtStdDevBuffer[i] =
            StdDev_Func(i, typical_price, ExtMovingBuffer, InpBandsPeriod);
        //--- upper line
        ExtUpperBuffer[i] =
            ExtMovingBuffer[i] + InpBandsDeviations * ExtStdDevBuffer[i];
        //--- lower line
        ExtLowerBuffer[i] =
            ExtMovingBuffer[i] - InpBandsDeviations * ExtStdDevBuffer[i];
        //---
...
//+------------------------------------------------------------------+
double StdDev_Func(int position, const double &price[],
                   const double &MAprice[], int period)
{
    //--- variables
    ...
    if (position >= period)
    {
        ...
        for (int i = 0; i < period; i++)
            StdDev_dTmp +=
                MathPow(price[position - i] - MAprice[position], 2);
        StdDev_dTmp = 1 / MathSqrt(StdDev_dTmp / period);
    }
